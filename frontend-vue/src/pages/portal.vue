<template>
  <div class="bg-grey-lighten-4">
    <v-container class="py-16">
      <h1 class="text-h3 text-center font-weight-bold mb-2">Welcome to our Mosque Learning Platform!</h1>
      <p class="text-center mb-12">Here you can make an application to join us or login if you have an account set up already.</p>

      <h2 class="text-center mb-8">User Portals</h2>
      <v-row justify="center">
        <v-col v-for="portal in portals" :key="portal.type" cols="12" md="4">
          <v-card class="pa-6 text-center" elevation="2" rounded="lg">
            <v-avatar class="mb-6" color="green-lighten-4" size="80">
              <v-icon color="green-darken-2" size="40">{{ portal.icon }}</v-icon>
            </v-avatar>

            <h2 class="text-h5 font-weight-bold mb-4">{{ portal.title }}</h2>
      ...
      <v-row justify="center">
        <v-col-12>
          <v-card class="pa-6 text-center" elevation="2" rounded="lg" to="/apply">
            <v-avatar class="mb-6" color="green-lighten-4" size="80">
...
<script setup lang="ts">
  ...
  function openLoginModal (portalType) {
    ...
    selectedPortalTitle.value = portals.find(p => p.type === portalType).title
